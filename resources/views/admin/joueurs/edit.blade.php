    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.joueurs.update", [$joueur->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_joueur') ? 'is-invalid' : '' }}" type="text" name="nom_joueur" id="nom_joueur" value="{{ old('nom_joueur', $joueur->nom_joueur) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prenom') ? 'is-invalid' : '' }}" type="text" name="prenom" id="prenom" value="{{ old('prenom', $joueur->prenom) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $joueur->email) }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nom_equipe') ? 'is-invalid' : '' }}" name="nom_equipe_id" id="nom_equipe_id" required>
                    @foreach($nom_equipes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nom_equipe_id') ? old('nom_equipe_id') : $joueur->nom_equipe->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
