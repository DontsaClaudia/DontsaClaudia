    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.equipes.update", [$equipe->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_equipe') ? 'is-invalid' : '' }}" type="text" name="nom_equipe" id="nom_equipe" value="{{ old('nom_equipe', $equipe->nom_equipe) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nom_coach') ? 'is-invalid' : '' }}" name="nom_coach_id" id="nom_coach_id" required>
                    @foreach($nom_coaches as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nom_coach_id') ? old('nom_coach_id') : $equipe->nom_coach->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
