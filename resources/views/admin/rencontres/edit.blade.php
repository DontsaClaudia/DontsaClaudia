...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.rencontre.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rencontres.update", [$rencontre->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipe_1') ? 'is-invalid' : '' }}" name="equipe_1_id" id="equipe_1_id" required>
                    @foreach($equipe_1s as $id => $entry)
                        <option value="{{ $id }}" {{ (old('equipe_1_id') ? old('equipe_1_id') : $rencontre->equipe_1->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipe_2') ? 'is-invalid' : '' }}" name="equipe_2_id" id="equipe_2_id" required>
                    @foreach($equipe_2s as $id => $entry)
                        <option value="{{ $id }}" {{ (old('equipe_2_id') ? old('equipe_2_id') : $rencontre->equipe_2->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_et_heure') ? 'is-invalid' : '' }}" type="text" name="date_et_heure" id="date_et_heure" value="{{ old('date_et_heure', $rencontre->date_et_heure) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resultat_1') ? 'is-invalid' : '' }}" type="number" name="resultat_1" id="resultat_1" value="{{ old('resultat_1', $rencontre->resultat_1) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resultat_2') ? 'is-invalid' : '' }}" type="number" name="resultat_2" id="resultat_2" value="{{ old('resultat_2', $rencontre->resultat_2) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('arbitre') ? 'is-invalid' : '' }}" name="arbitre_id" id="arbitre_id">
                    @foreach($arbitres as $id => $entry)
                        <option value="{{ $id }}" {{ (old('arbitre_id') ? old('arbitre_id') : $rencontre->arbitre->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
